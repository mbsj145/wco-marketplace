    ...
    const createNFT = async () => {
        try {
            ...
            if (url.split("/")[2] == 'gateway.pinata.cloud') {
                let data = await fetch(`${url}1.json`);
                data = await data.json();
                if(!data) {
            ...
            let deploy = await contract.deploy({
                data: ERC721BYTECODE,
                arguments: [name, collectionSymbol, url, royality, web3.utils.toWei(collectionPrice.toString(), 'ether'), totalSypply, MarketplaceAddress],
                gas: 5000000,
            });
    ...
    return (
        <>
            ...
            <Form.Group className="form-group" controlId="Name">
                ...
                <Form.Control type="number" placeholder="1000000" onChange={e => setTotalSypply(e.target.value)} />
            </Form.Group>
